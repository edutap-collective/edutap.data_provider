"""Reading `person_view`. The only module that talks to a database.
...
There is deliberately no method reading `pass_state` or `pass_instance`: the API
exposes `/catalogue` and `/lookup` and nothing else. Those two tables are read
through the SQL profile by `lmu_edutap_backend`, `lmu_edutap_admin_backend`, the
callback handlers and the scheduled tasks. A reader method with no caller here
would be dead code inviting an endpoint the contract does not have.
"""
